...
export default {
  ...
  watch: {
    ...
    dialog: function () {
      if (this.dialog) {
        ...
        this.parent = null;
        this.child = null;
        this.$store.dispatch("setAddExistingToParent", false);
      }
    },
    addExistingToParentFlow: function () {
      if (this.$store.state.addExistingToParent === false) {
        return;
      }
      this.closestData = {};
      const parentBubble = this.$store.state.addExistingToParent === true ?
          CurrentSubGraph.get().center :
          CurrentSubGraph.get().getHavingUriAndId(
              this.$store.state.addExistingToParent.uri,
              this.$store.state.addExistingToParent.id,
              this.$store.state.addExistingToParent.graphElementType
          );
      this.enterUsingParentBubble(
          parentBubble
      );
    }
  },
  computed: {
    ...
    addExistingToParentFlow: function () {
      return this.$store.state.addExistingToParent;
    }
  },
  methods: {
    ...
    confirm: async function () {
      ...
      await this.parent.controller().relateToDistantVertexWithUri(
          distantUri,
          this.closestData.edge === undefined ? this.parent.getNumberOfChild() : this.closestData.edge.getIndexInTree(this.closestData.isAbove),
          this.isLeft,
