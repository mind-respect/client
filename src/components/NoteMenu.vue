<template>
    <v-card flat @click="$store.dispatch('setIsDescriptionFlow', true)" hover
        ...
        <v-card-title class="pt-4 pb-0 subtitle-1">
            <span class="pt-0 grey--text body-1 text-center" v-show="bubble && bubble.getComment() === ''">
                {{$t('noteMenu:noNote')}}
            </span>
        </v-card-title>
        <v-card-text v-if="bubble" v-html="bubble.getComment()" class="pt-0" style="color:inherit"></v-card-text>
        <v-skeleton-loader
                type="paragraph"
                class="pa-6"
